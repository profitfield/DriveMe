...
export class OrdersService {
   ...
   async getAvailableOrders(carClass?: string, maxDistance?: number): Promise<Order[]> {
      ...
      if (maxDistance) {
          // В реальном проекте здесь будет геопространственный запрос
          // Для MVP используем упрощенную версию
      }
   
      return query
          .orderBy('order.pickupDatetime', 'ASC')
          .getMany();
   }
